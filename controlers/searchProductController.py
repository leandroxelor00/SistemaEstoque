        ...
        for produto in self.dao.viewList():
            if not self.tela.searchBar.value:
                ...
                if len(self.tela.searchBar.value) == 1:
                    if (self.tela.searchBar.value == str(produto["idProd"])) or (produto["nome"][0] == self.tela.searchBar.value[0]) or (produto["marca"][0] == self.tela.searchBar.value[0]):
                        self.addLinhas(produto)

                else:
                    if (self.tela.searchBar.value == str(produto["idProd"])) or (produto["nome"] == self.tela.searchBar.value) or (produto["marca"] == self.tela.searchBar.value):
                        self.addLinhas(produto)
